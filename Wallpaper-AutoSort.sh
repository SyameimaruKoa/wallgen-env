...
# --- 結果集計 ---
count_gears3=$(( $(grep "^GearS3" "$TMP_LOG" 2>/dev/null | wc -l) + 0 ))
count_phone=$(( $(grep "^スマホ$" "$TMP_LOG" 2>/dev/null | wc -l) + 0 ))
count_phone_low=$(( $(grep "^スマホ_S22U未満" "$TMP_LOG" 2>/dev/null | wc -l) + 0 ))
count_ipad=$(( $(grep "^iPad Pro$" "$TMP_LOG" 2>/dev/null | wc -l) + 0 ))
count_ipad_low=$(( $(grep "^iPad Pro_2388未満" "$TMP_LOG" 2>/dev/null | wc -l) + 0 ))
count_invalid=$(( $(grep "^不正ファイル" "$TMP_LOG" 2>/dev/null | wc -l) + 0 ))
count_pc_4k=$(( $(grep "^パソコン$" "$TMP_LOG" 2>/dev/null | wc -l) + 0 ))
count_pc_mid=$(( $(grep "^パソコン_4K未満" "$TMP_LOG" 2>/dev/null | wc -l) + 0 ))
count_pc_low=$(( $(grep "^パソコン_FHD未満" "$TMP_LOG" 2>/dev/null | wc -l) + 0 ))
skip_count=$(( $(grep "^Skip:" "$TMP_LOG" 2>/dev/null | wc -l) + 0 ))
